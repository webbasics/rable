...
var funcs = ['isBoolean', 'isString', 'isNumber', 'isFunction', 'isArray', 'isObject', 'isHTMLElement', 'isNodeList', 'isHTMLCollection', 'is'];
funcs.forEach(function(func) {
    Boolean.prototype[func] =
        ...
        HTMLCollection.prototype[func] = function() {
...
Array.prototype.isset =
    ...
    HTMLCollection.prototype.isset = function(val, search_val = false) {
...
HTMLElement.prototype.child = function(chld = 0, elementsOnly = true) { //child ~> child
    var child = (elementsOnly ? this.children : this.childNodes);
...
HTMLElement.prototype.on = function(type, action, capture = false) {
    this.addEventListener(type, action, capture);
    return this;
}

NodeList.prototype.on = function(type, action, val3, val4) {
    if (val3 !== undefined) {
        if (val3.isNumber()) {
            var item = val3;
            var capture = false;
        } else if (val3.isBoolean()) {
            var item = false;
            var capture = val3;
        }
    } else {
    ...
    if (val4 !== undefined) {
        if (val4.isNumber()) {
            var item = val4;
        } else if (val4.isBoolean()) {
            var capture = val4;
        }
    }

    if (item.isNumber() && this.isset(item)) {
        this[item].on(type, action, capture);
    } else {
        this.forEach(function(elmnt) {
            elmnt.on(type, action, capture);
        });
    }
}
...
Object.getOwnPropertyNames(document).concat(Object.getOwnPropertyNames(Object.getPrototypeOf(Object.getPrototypeOf(document)))).concat(Object.getOwnPropertyNames(Object.getPrototypeOf(window))).filter(function(i){return !i.indexOf('on')&&(document[i]==null||typeof document[i]=='function');}).filter(function(elem, pos, self){return self.indexOf(elem) == pos;}).forEach(function(eventType) {
    HTMLElement.prototype[eventType.slice(0, 2) + eventType.charAt(2).toUpperCase() + eventType.slice(3)] = function(action, capture = false) {
        return this.on(eventType.slice(2), action, capture);
    }

    NodeList.prototype[eventType.slice(0, 2) + eventType.charAt(2).toUpperCase() + eventType.slice(3)] = function(action, val2, val3) {
        return this.on(eventType.slice(2), action, val2, val3);
    }
});
...
_.isHTMLCollection = function(obj) {
    if (obj.is() === "HTMLCollection") {
        return true;
    } else {
        return false;
    }
...
_.ready = function(func, val2, val3) {
    if (val2 !== undefined) {
        if (val2.isNumber()) {
            var item = val2;
            var obj = false;
    ...
    if (obj === document || obj === 'document' || obj === 'doc' || !doc) {
        document.addEventListener('DOMContentLoaded', func);
    } else if (obj.isHTMLElement()) {
        obj.addEventListener('DOMContentLoaded', func);
    } else if (obj.isNodeList()) {
        if (item.isNumber() && obj.isset(item)) {
...
_.isset = function(key, obj, search_val = false) {
    ...
    } else if (obj.isNodeList() === true || obj.isHTMLCollection() === true) {
...
_.on = function(type, action, capture = false) {
    document.addEventListener(type, action, capture);
    return true;
}
